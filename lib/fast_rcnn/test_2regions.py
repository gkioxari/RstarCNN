...
def im_detect(net, im, boxes, gt_label):
    ...
    sec_boxes_overlaps = \
      utils.cython_bbox.bbox_overlaps(sec_boxes.astype(np.float), 
                                      sec_boxes.astype(np.float))

    selected_boxes = np.zeros((scores.shape[1], 8, gt_boxes.shape[0]))
    ...
    for i in xrange(gt_boxes.shape[0]):
        ...
        if keep_inds.size > 0:
            ...
            selected_boxes[:,0:4,i] = sec_boxes[keep_inds[winner_ind]]
            secondary_ind1 = keep_inds[winner_ind]

            # second auxiliary region
            for a in xrange(scores.shape[1]):
                keep_inds2 = np.where((boxes_overlaps[:,i]>=cfg.TEST.IOU_LB) &
                                      (boxes_overlaps[:,i]<=cfg.TEST.IOU_UB) &
                                      (sec_boxes_overlaps[:,secondary_ind1[a]]>=cfg.TEST.IOU_LB) &
                                      (sec_boxes_overlaps[:,secondary_ind1[a]]<=cfg.TEST.IOU_UB))[0]
                if keep_inds2.size>0:
                    this_scores = np.amax(secondary_scores[keep_inds2,a], axis=0)  
                    scores[i,a] = scores[i,a]+this_scores
                    winner_ind2  = np.argmax(secondary_scores[keep_inds2,a], axis=0)
                    selected_boxes[a,4:8,i] = sec_boxes[keep_inds2[winner_ind2]]
...
def test_net(net, imdb):
    ...
    all_selected_boxes = np.zeros((num_classes, 8, 0))
