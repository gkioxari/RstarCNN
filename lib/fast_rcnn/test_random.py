...
def _get_blobs(im, rois, sec_rois):
    """Convert an image and RoIs within that image into network inputs."""
    blobs = {'data' : None, 'rois' : None, 'secondary_rois': None}
    blobs['data'], im_scale_factors = _get_image_blob(im)
    blobs['rois'] = _get_rois_blob(rois, im_scale_factors)
    blobs['secondary_rois'] = _get_rois_blob(sec_rois, im_scale_factors)
...
def im_detect(net, im, boxes, gt_label):
    ...
    sec_rois = np.zeros((0, 4), dtype=np.int64)
    
    for i in xrange(gt_boxes.shape[0]):
        cinds = np.where((boxes_overlaps[:,i] >= cfg.TEST.IOU_LB) &
                         (boxes_overlaps[:,i] <= cfg.TEST.IOU_UB))[0]
        if cinds.size>0:
            cinds = npr.choice(cinds, size = 1, replace=False)
            sec_rois = np.concatenate((sec_rois, sec_boxes[cinds]), axis=0)
        else:
            sec_rois = np.concatenate((sec_rois, np.array(gt_boxes[i], ndmin=2)), axis=0)


    num_rois = gt_boxes.shape[0]
    num_sec_rois = sec_rois.shape[0]
    assert num_rois==num_sec_rois

    blobs, unused_im_scale_factors = _get_blobs(im, gt_boxes, sec_rois)
...
def test_net(net, imdb):
    ...
    for i in xrange(num_images):
        ...
        _t['im_detect'].tic()
        scores = im_detect(net, im, roidb[i]['boxes'], roidb[i]['gt_classes'])
        _t['im_detect'].toc()
